...
from not_real_drop.modify_llama import convert_kvcache_llama_heavy_recent, LlamaAttention_heavy_hitter
...
def main():
    ...
    """
    CUDA_VISIBLE_DEVICES=0 python  run_text_generation_un_drop.py --model_arch llama --model_name meta-llama/Llama-2-7b-chat-hf 
    """
    ...
    prompt_text = 'Southern University of Science and Technology is what ？ Give me 5000 words '
    ...
    generate_ids = model.generate(input_ids, max_new_tokens=5000)
    result = tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
    print("################## Generated Context with Full Cache ###################")
    print(result)
    print('full time is ', time.time()- full_time)
    ...
    model.half().eval().cuda()
    h2o_start = time.time()
    generate_ids_hh = model.generate(input_ids, max_new_tokens=5000)  
    ...
    print(result_hh)
    print('H2O time is ', time.time() - h2o_start)
